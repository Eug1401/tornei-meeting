const fs = require('fs');
const vm = require('vm');
const path = '/mnt/data/work_v130/tornei-clean/assets/js/store.js';
...
test('code regression surface: public team PDF excludes all knockout phases',()=>{const publicCode=fs.readFileSync('/mnt/data/work_v130/tornei-clean/assets/js/public.js','utf8'); assert(publicCode.includes('store.isKnockoutPhase?store.isKnockoutPhase(m)'), 'public team match list uses isKnockoutPhase guard'); const adminCommon=fs.readFileSync('/mnt/data/work_v130/tornei-clean/assets/js/admin-common.js','utf8'); assert(adminCommon.includes('calculateStandings(s)'), 'admin dashboard still calls selector, which now defaults to main standings');});

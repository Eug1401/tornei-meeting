(function(){
  ...
  function statsGrid(stats){return `<div class="stat"><strong>${stats.teams}</strong><span>Squadre</span></div><div class="stat"><strong>${stats.players}</strong><span>Giocatori</span></div><div class="stat"><strong>${stats.matches}</strong><span>Partite</span></div><div class="stat"><strong>${stats.goals}</strong><span>Gol</span></div><div class="stat"><strong>${stats.yellow}</strong><span>Gialli</span></div><div class="stat"><strong>${stats.red}</strong><span>Rossi</span></div>`;}
  ...
  function articleImageSrc(article){
    return String(article?.image||article?.imageUrl||article?.coverImage||article?.cover||article?.photo||article?.thumbnail||'').trim();
  }
  ...
  window.NexoraUI={esc,$,$$,logo,siteTitle,siteSubtitle,siteLogoMarkup,applySiteTheme,fmtDate,teamOptions,playerOptions,statsGrid,standingsTable,groupStandingsSelector,groupStandingsTables,playerStatsTable,matchStatusMeta,matchCard,matchList,teamGrid,rulesSummary,bracketMarkup,articleCard,articleDetail,articleList,articlePlaceholder,replaceBrokenArticleImage,bindTabs,bindDisclosures};
})();
